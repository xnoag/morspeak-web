...
const DATES: { date: string; label: string; slots: string[] }[] = [
  { date: '2026-06-29', label: '6월 29일 (월)', slots: ['09:00','09:30','10:00','10:30','11:00','15:00','15:30','16:00','16:30','17:00','17:30'] },
  ...
  return (
    <div style={{ minHeight:'100svh', background:'#F7F7F9', fontFamily:F }}>
      ...
      <div style={{ maxWidth:520, margin:'0 auto', padding:'20px 16px 40px' }}>
        ...
        {/* 가능한 시간 없을 때 */}
        {!hasAvailable && !selected && (
          <div style={{ background:'#fff', borderRadius:14, padding:20, marginTop:4, boxShadow:'0 2px 10px rgba(0,0,0,0.06)' }}>
            {altDone ? (
              ...
              <>
                <p style={{ fontSize:14, fontWeight:600, color:'#1C1C1E', marginBottom:4 }}>혹시 가능한 시간이 없으신가요?</p>
                <p style={{ fontSize:13, color:'#8E8E93', marginBottom:14, lineHeight:1.5 }}>
                  아래에 가능하신 시간을 입력해주시면 조율해보겠습니다.
                </p>
                <div style={{ marginBottom:10 }}>
                  ...
                  <input value={altPhone} onChange={e=>setAltPhone(e.target.value)} placeholder="연락처 (010-0000-0000)"
                    style={{ width:'100%', padding:'10px 14px', border:'1.5px solid #E5E5EA', borderRadius:10, fontSize:14, outline:'none', fontFamily:F, boxSizing:'border-box' as const, marginBottom:8 }} />
                  <input value={altTime} onChange={e=>setAltTime(e.target.value)} placeholder="예: 6/29 오후 2시, 6/30 오전 중"
                    style={{ width:'100%', padding:'10px 14px', border:'1.5px solid #E5E5EA', borderRadius:10, fontSize:14, outline:'none', fontFamily:F, boxSizing:'border-box' as const }} />
                </div>
                <button onClick={handleAlt} disabled={altSubmitting || !altTime.trim() || !altName.trim() || !altPhone.trim()}
                  style={{ width:'100%', padding:'11px', borderRadius:10, border:'none',
                    background:(!altTime.trim()||!altName.trim()||!altPhone.trim()||altSubmitting)?'#C7C7CC':'#1C1C1E',
