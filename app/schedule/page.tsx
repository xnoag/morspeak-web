  ...
  const handleBook = async () => {
    ...
    try {
      await runTransaction(db, async (tx) => {
        ...
        if (snap.exists() && (snap.data() as Booking).patientName) throw new Error('ALREADY_BOOKED');
        tx.set(slotDoc, {
  ...
  return (
    <div style={{ minHeight:'100svh', background:'#F5F5F7', fontFamily:F }}>
      ...
      <div style={{ maxWidth:540, margin:'0 auto', padding:'24px 16px 48px' }}>
        ...
        {/* SECTION 3: 문의 전화 */}
        <div style={{ background:'#fff', border:'2px solid #E5E5EA', borderRadius:20, padding:'22px 20px', textAlign:'center' }}>
          <p style={{ fontSize:17, fontWeight:700, color:'#1C1C1E', marginBottom:6 }}>원하시는 시간이 없으신가요?</p>
          <p style={{ fontSize:15, color:'#6E6E73', marginBottom:18, lineHeight:1.6 }}>
            아래 번호로 전화 주시면<br/>일정을 조율해드리겠습니다.
          </p>
