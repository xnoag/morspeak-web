...
export default function Dashboard() {
  ...
  async function toggleInternal(p: Patient) {
    const next = !p.isInternal
    setPatients(prev => prev.map(x => x.id === p.id ? { ...x, isInternal: next } : x))
    await updateDoc(doc(getDb(), 'usageStats', p.id), { isInternal: next })
  }
  ...
  const realFiltered = filtered.filter(p => !p.isInternal)
  const internalFiltered = filtered.filter(p => p.isInternal)

  const realPatients = patients.filter(p => !p.isInternal)
  const activeToday = realPatients.filter(p => isActiveToday(p.lastUpdated)).length
  const totalSpeak = realPatients.reduce((s, p) => s + (p.speakCount ?? 0), 0)
  const totalCall  = realPatients.reduce((s, p) => s + (p.callCount ?? 0), 0)
  const totalSec   = realPatients.reduce((s, p) => s + (p.totalSessionSeconds ?? 0), 0)
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '24px 28px' }}>
        ...
        {!loading && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 14, marginBottom: 24 }}>
            {[
              { label: '총 환우', value: `${realPatients.length}명`, sub: `오늘 활동 ${activeToday}명`, color: '#007AFF', icon: '👤' },
        ...
        {!loading && (
          <PatientTable
            title="환우 목록" list={realFiltered} sortKey={sortKey} sortAsc={sortAsc} onSort={sort}
            emptyLabel="검색 결과 없음"
            toggleLabel="내부용으로 표시" onToggleInternal={toggleInternal}
          />
        )}

        {/* 내부용 계정 — 실제 환우가 아닌 테스트/교육/시연용 계정 */}
        {!loading && internalFiltered.length > 0 && (
          <div style={{ marginTop: 20 }}>
            <PatientTable
              title="내부용 계정" list={internalFiltered} sortKey={sortKey} sortAsc={sortAsc} onSort={sort}
              emptyLabel="검색 결과 없음"
              toggleLabel="환우로 표시" onToggleInternal={toggleInternal}
              muted
            />
          </div>
        )}
        ...
        {loading && (
          <div style={{ background: '#fff', borderRadius: 16, border: '1px solid rgba(0,0,0,0.07)', padding: 56, textAlign: 'center', color: '#8e8e93', fontSize: 13 }}>불러오는 중...</div>
        )}
      </div>
    </div>
  )
...
function PatientTable({ title, list, sortKey, sortAsc, onSort, emptyLabel, toggleLabel, onToggleInternal, muted }: {
  title: string
  list: Patient[]
  sortKey: SortKey
  sortAsc: boolean
  ...
  emptyLabel: string
  toggleLabel: string
  onToggleInternal: (p: Patient) => void
  muted?: boolean
}) {
  return (
    <div style={{ background: '#fff', borderRadius: 16, border: '1px solid rgba(0,0,0,0.07)', boxShadow: '0 1px 4px rgba(0,0,0,0.05)', overflow: 'hidden', opacity: muted ? 0.85 : 1 }}>
      <div style={{ padding: '14px 20px', borderBottom: '1px solid #f2f2f7', display: 'flex', alignItems: 'center', gap: 8 }}>
        <span style={{ fontWeight: 700, fontSize: 15, color: muted ? '#6e6e73' : '#1d1d1f' }}>{title}</span>
        <span style={{ fontSize: 12, color: '#8e8e93', background: '#f2f2f7', padding: '2px 8px', borderRadius: 20 }}>{list.length}명</span>
      </div>

      {list.length === 0 ? (
        <div style={{ padding: 56, textAlign: 'center', color: '#8e8e93', fontSize: 13 }}>{emptyLabel}</div>
      ) : (
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ background: '#fafafa', borderBottom: '1px solid #f2f2f7' }}>
              ...
              <th style={{ padding: '10px 20px', width: 100 }} />
            </tr>
          </thead>
          <tbody>
            {list.map((p, i) => {
              ...
              return (
                <tr key={p.id} onClick={() => location.href = `/tracking/patients/${p.userCode}`}
                  ...
                  <td style={{ padding: '13px 20px', textAlign: 'right' }}>
                    <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); onToggleInternal(p) }}
                      style={{ padding: '4px 10px', borderRadius: 6, border: '1px solid #d2d2d7', background: '#fff', color: '#6e6e73', fontSize: 10.5, cursor: 'pointer', whiteSpace: 'nowrap' }}>
                      {toggleLabel}
                    </button>
                  </td>
                </tr>
